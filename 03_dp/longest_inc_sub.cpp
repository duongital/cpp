...
vector<int> longestIncreasingSubsequence(vector<int>& nums) {
    int n = nums.size();
    // Initialize the length array with 1, as each element is a subsequence of length 1
    vector<int> length(n, 1);
    // Initialize the previous array to track the indices of the elements that make up the longest increasing subsequence
    vector<int> prev(n, -1);
